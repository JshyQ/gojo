...
// ─── Command .pins / .pinsearch / .caripinterest ───────────────────────────
// Cari gambar di Pinterest lewat API siputzx (bukan download dari URL).
// Beda dengan .pin/.pinterest yang butuh link pin.it / pinterest.com.
const PINTEREST_SEARCH_API = 'https://api.siputzx.my.id/api/s/pinterest';

async function searchPinterest(ctx) {
    ...
    if (!query) {
        return reply(
            ...
            `Cara pakai:\n` +
            `• \`.pins <kata kunci>\`\n` +
            `• \`.pinsearch <kata kunci>\`\n` +
            `• \`.caripinterest <kata kunci>\`\n\n` +
            `Contoh:\n` +
            `• \`.pins cat\`\n` +
            `• \`.pins anime girl\`\n` +
            `• \`.pins wallpaper aesthetic\`\n\n` +
            `_Note: \`.pin\` (tanpa s) = download dari link Pinterest._`
        );
    }

    try {
        const url = `${PINTEREST_SEARCH_API}?query=${encodeURIComponent(query)}&type=image`;
        ...
        if (!res.ok) {
            return reply(`❌ Gagal mencari di Pinterest (HTTP ${res.status}). Coba lagi nanti.`);
        }

        const json = await res.json();
        const list = Array.isArray(json?.data) ? json.data : [];
        const images = list.filter((item) => item?.image_url && typeof item.image_url === 'string');

        if (!images.length) {
            return reply(`🔍 Tidak ditemukan gambar untuk: *${query}*\nCoba kata kunci lain.`);
        }

        // Ambil 1 gambar acak dari hasil
        const pick = images[Math.floor(Math.random() * images.length)];
        const title = (pick.grid_title || pick.seo_alt_text || pick.description || query).toString().trim().slice(0, 120);
        const pinUrl = pick.pin || pick.link || '';
        const caption =
            `📌 *Pinterest Search*\n` +
            `🔍 Query: *${query}*\n` +
            (title ? `📝 ${title}\n` : '') +
        ...
        await sock.sendMessage(
            jid,
            { image: { url: pick.image_url }, caption },
            { quoted: msg }
        );
    } catch (err) {
        if (err?.name === 'AbortError') {
            return reply('⏳ Timeout: API Pinterest terlalu lama merespons. Coba lagi.');
        }
        console.error('[searchPinterest]', err?.message || err);
        return reply('❌ Gagal mencari gambar Pinterest. Coba lagi nanti.');
    }
}
